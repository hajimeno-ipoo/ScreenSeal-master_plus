...
7. `PointerTrackingService` がカーソル位置とクリック状態を追跡し、クリック時ズームに使います。
8. メニューの `Capture Mode` で、録画かスクリーンショットかを切り替えます。
9. 録画メニューから、カーソルのハイライト色とクリックリング色も変えられます。
10. 録画開始前には、対象画面の中央に3秒カウントダウンが出ます。
11. 撮影対象は、全画面・選択ウィンドウ・選択範囲から選べます。
